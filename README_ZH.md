...
Yuan3.0 Ultra 结合改进的 RIRM（反思抑制奖励机制），形成以 Fast-thinking 为核心的Reflection-aware Adaptive Policy Optimization（RAPO）训练策略，在大规模强化学习过程中实现精度提升与推理 token 持续下降，使模型能力扩展与计算效率同步优化。同时，我们发布了Yuan3.0 Ultra模型的<a href="https://arxiv.org/abs/2601.01718" target="_blank">**技术报告**</a>，可以通过论文查看更详细的技术细节与测评结果。
...
⚡ **快思考强化学习（RAPO）**  ：通过反思抑制奖励机制，在大规模强化学习过程中实现“精度显著提升 + 推理长度持续下降”的同步优化，使模型更加高效、克制且稳定。
...
在 Fast-thinking RL 阶段，模型易对数学、科学推理任务产生过多反思步骤（overthinking）。Yuan3.0 Ultra 在 Yuan3.0 Flash 的 RAPO 框架基础上改进训练机制：正确样本反思步骤越少奖励越高，错误样本反思步骤越多惩罚越重，改进后训练准确率提升 **16.33%**，输出 token 长度降低 **14.38%**，AIME 2024 / MATH-500 分别达到 **47.8%** / **93.1%**。
...
Spider 1.0 和 BIRD 是 Text-to-SQL 领域的两项权威基准。Yuan3.0 Ultra 在 Spider 1.0 上以 **83.9%** 实现精度领先，在 BIRD 评测上表现同样出色。
