    ...
    def load_state_dict(self, state_dict):
        ...
        ** Note: Torch optimizer's state structure. **
        ...
        if len(self.optimizer.state) == 0:
            ...
            state_dict_state = []
            if self.config.main_params_dtype == torch.float32 and self.config.params_dtype == torch.bfloat16:
                master_param_dtype = torch.int16
            else:
                master_param_dtype = torch.float32
            if self.config.use_offload_precision_aware_optimizer == True:
                param_device = "cpu"
            else:
                    ...
                    for gbuf_range_map in gbuf_range_map_for_all_buckets:
                        for model_param, param_range_map in gbuf_range_map["param_map"].items():
                            ...
                            init_shard = lambda: torch.empty(
                                (numel,), dtype=torch.float32, device=param_device
                            )

                            tensors = {"exp_avg": init_shard(), "exp_avg_sq": init_shard()}
                            if self.config.use_precision_aware_optimizer:
                                tensors["master_param"] = torch.empty(
                                            (numel,), dtype=master_param_dtype, device=param_device
                                            )
