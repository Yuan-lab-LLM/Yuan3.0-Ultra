...
def _init_args():
    ...
    parser.add_argument("--model_type", type=str, choices=["auto", "yuan", "yuanvl"], help="The mode type for the conversion")
...
@torch.inference_mode()
def convert_checkpoint_from_transformers_to_megatron_yuan(
    hfmodel,
    mgmodel,
    hf_config,
    tfconfig,
    layer_start_end: Optional[tuple[int, int]] = None,
    has_attn_router=False,
):
    ...
    # layermaps = zip(mgllm.decoder.layers, hfllm.layers, strict=True)
    # for mglayer, hflayer in layermaps:
    for layer_idx, (mglayer, hflayer) in enumerate(
        zip(mgmodel.decoder.layers, hfmodel.model.layers[layer_start:layer_end], strict=True)
    ):
        # input layernorm
        print(mglayer.input_layernorm)
        print(hflayer.input_layernorm)
        copied_numel += safe_copy(hflayer.input_layernorm.weight, mglayer.input_layernorm.weight)

        # qk,v,o
        copied_numel += safe_copy(hflayer.self_attn.get_query_key.weight, mglayer.self_attention.linear_qk.weight)
        copied_numel += safe_copy(hflayer.self_attn.v_proj.weight, mglayer.self_attention.linear_v.weight)
        copied_numel += safe_copy(hflayer.self_attn.o_proj.weight, mglayer.self_attention.linear_proj.weight)

        # lf_gate
        copied_numel += safe_copy(hflayer.self_attn.lf_gate.conv1.weight, mglayer.self_attention.lf_gate.conv1.weight)
        copied_numel += safe_copy(hflayer.self_attn.lf_gate.conv1.bias, mglayer.self_attention.lf_gate.conv1.bias)
        copied_numel += safe_copy(hflayer.self_attn.lf_gate.conv2.weight, mglayer.self_attention.lf_gate.conv2.weight)
        copied_numel += safe_copy(hflayer.self_attn.lf_gate.conv2.bias, mglayer.self_attention.lf_gate.conv2.bias)
        copied_numel += safe_copy(hflayer.self_attn.lf_gate.output_layernorm.weight, mglayer.self_attention.lf_gate.output_layernorm.weight)

        # post attn layernorm
        copied_numel += safe_copy(hflayer.post_attention_layernorm.weight, mglayer.post_attention_layernorm.weight)
        # attn router
        if hf_config.moe_config.router_type == 'attn_router':
            copied_numel += safe_copy(hflayer.mlp.router.query_key_value.weight, mglayer.mlp.router.attention_router.weight)
        else:
            copied_numel += safe_copy(hflayer.mlp.router.weight, mglayer.mlp.linear_fc1.weight)

        # MOE

        if hasattr(hf_config.moe_config, 'per_layer_experts_blocks'):
            num_experts = hf_config.moe_config.per_layer_experts_blocks[layer_idx]
        elif hasattr(hf_config.moe_config, 'num_experts'):
            num_experts = hf_config.moe_config.num_experts  
        assert num_experts == len(hflayer.mlp.experts.w1)

        if tfconfig.moe_grouped_gemm:
            for i, (hf_expert_w1, hf_expert_w2) in enumerate(zip(hflayer.mlp.experts.w1, hflayer.mlp.experts.w2)):
                linear_fc1_weighti = getattr(mglayer.mlp.experts.linear_fc1, "weight" + str(i))
                numel += safe_copy(hf_expert_w1.weight, linear_fc1_weighti)
                linear_fc2_weighti = getattr(hf_expert.mlp.experts.linear_fc2, "weight" + str(i))
                numel += safe_copy(hf_expert_w2.weight, linear_fc2_weighti)
        else:
            for i, hf_expert in enumerate(hflayer.mlp.experts):
                expert = mglayer.mlp.experts.local_experts[i]
                numel += safe_copy(hflayer.mlp.experts.w1, expert.linear_fc1.weight)
                numel += safe_copy(hflayer.mlp.experts.w2, expert.linear_fc2.weight)
...
@torch.inference_mode()
def convert_checkpoint_from_transformers_to_megatron_yuan_vl(hfmodel, mgmodel, hf_config):
    mgmodel = mgmodel.bfloat16()
    hfmodel = hfmodel.bfloat16()

    # 1. vision model
    hfvision = hfmodel.vision_model
    mgvision = mgmodel.vision_model
    copied_numel = 0
    copied_numel += safe_copy(hfvision.embeddings.class_embedding, mgvision.rembeddings.class_embedding)
    copied_numel += safe_copy(hfvision.embeddings.patch_embed.weight, mgvision.embeddings.patch_embed.weight)
    copied_numel += safe_copy(hfvision.embeddings.patch_embed.bias, mgvision.embeddings.patch_embed.bias)
    copied_numel += safe_copy(hfvision.embeddings.position_embedding, mgvision.embeddings.position_embedding)

    for hfblock, mgblock in zip(hfvision.encoder.layers, mgvision.encoder.layers, strict=True):
        # norm1 --> linear_qkv.norm
        copied_numel += safe_copy(hfblock.attn.proj.weight, mgblock.attn.proj.weight)
        copied_numel += safe_copy(hfblock.attn.proj.bias, mgblock.attn.proj.bias)

        # norm2 --> mlp.linear_fc1.norm
        copied_numel += safe_copy(hfblock.norm1.weight, mgblock.norm1.weight)
        copied_numel += safe_copy(hfblock.norm1.bias, mgblock.norm1.bias)
        copied_numel += safe_copy(hfblock.norm2.weight, mgblock.norm2.weight)
        copied_numel += safe_copy(hfblock.norm2.bias, mgblock.norm2.bias)

        # qkv --> self_attention.linear_qkv
        copied_numel += safe_copy(hfblock.attn.qkv.weight, mgblock.attn.qkv.weight)
        copied_numel += safe_copy(hfblock.attn.qkv.bias, mgblock.attn.qkv.bias)
        copied_numel += safe_copy(hfblock.ls1, mgblock.ls1)
        copied_numel += safe_copy(hfblock.ls2, mgblock.ls2)

        # mlp --> mlp: gate
        copied_numel += safe_copy(hfblock.mlp.fc1.weight, mgblock.mlp.fc1.weight)
        copied_numel += safe_copy(hfblock.mlp.fc1.bias, mgblock.mlp.fc1.bias)
        copied_numel += safe_copy(hfblock.mlp.fc2.weight, mgblock.mlp.fc2.weight)
        copied_numel += safe_copy(hfblock.mlp.fc2.bias, mgblock.mlp.fc2.bias)

    n_params = sum([t.numel() for t in hfvision.state_dict().values()])
    assert n_params == copied_numel, f"n_params={n_params} != copied_numel={copied_numel}"

    # 2. vision projector
    hfimagemlp = hfvision.imagemlp
    mgimagemlp = mgvision.imagemlp
    copied_numel += safe_copy(hfimagemlp.up_proj.weight, mgimagemlp.up_proj.weight)
    copied_numel += safe_copy(hfimagemlp.gate_proj.weight, mgimagemlp.gate_proj.weight)
    copied_numel += safe_copy(hfimagemlp.down_proj.weight, mgimagemlp.down_proj.weight)
    # 3. vision projector layernorm
    copied_numel += safe_copy(hfvision.imagemlp_layernorm.weight, mgvision.imagemlp_layernorm.weight)

    # 4. llm [just Yuan]
    hfllm = hfmodel.language_model
    mgllm = mgmodel.language_model
    copied_numel += safe_copy(hfllm.embed_tokens.weight, mgllm.embedding.word_embeddings.weight)
    for (mglayer, hflayer) in zip(mgllm.decoder.layers, hfllm.model.layers[layer_start:layer_end], strict=True):
        # input layernorm
        copied_numel += safe_copy(hflayer.input_layernorm.weight, mglayer.input_layernorm.weight)
...
def convert_hf_to_mcore(hf_model_path, output_path, use_cpu_initialization=False, test=False, trust_remote_code=False, model_type="auto"):
    ...
    # 此处需要修改为不进行分布式初始化的版本，根据mpu修改分布式策略
    ...
    if model_type == 'auto':
        ...
        hf_config = YuanConfig.from_pretrained(hf_model_path, trust_remote_code=trust_remote_code)
    elif model_type == 'yuanvl':
        from verl.models.configs.configuration_yuanvl import YuanVLConfig
        hf_config = YuanVLConfig.from_pretrained(hf_model_path, trust_remote_code=trust_remote_code)
    ...
    elif "YuanForCausalLM" in hf_config.architectures:
        from verl.models.yuan2.modeling_yuan2 import YuanForCausalLM
        hf_model = YuanForCausalLM.from_pretrained(
            hf_model_path, torch_dtype=torch.bfloat16, trust_remote_code=trust_remote_code
        )
    elif "YuanVLChatModel" in hf_config.architectures:
        from verl.models.yuan2.modeling_yuanvl import YuanVLChatModel
        hf_model = YuanVLChatModel.from_pretrained(
            hf_model_path, torch_dtype=torch.bfloat16, trust_remote_code=trust_remote_code
        )
    else:
        hf_model = YuanVLChatModel.from_pretrained(
    ...
    if world_size > 1 and support_distributed_convert(hf_config):
        ...
        elif "YuanForForCausalLM" in hf_config.architectures:
            print(model[0])
            numel_partial: int = convert_checkpoint_from_transformers_to_megatron_yuan(
                hf_model, model[0].module, hf_config, layer_start_end=(layer_start, layer_end)
            )
        elif "YuanVLChatModel" in hf_config.architectures:
            numel_partial: int = convert_checkpoint_from_transformers_to_megatron_yuan_vl(
                hf_model, model[0].module, hf_config, layer_start_end=(layer_start, layer_end)
            )
        else:
            raise NotImplementedError(f"Distributed conversion is not supported for {hf_config.architectures} yet.")
    ...
    # load hf state dict to megatron model
        ...
        print(model[0])
        convert_checkpoint_from_transformers_to_megatron_yuan(hf_model, model[0].module, hf_config, tfconfig=tfconfig)
    elif "YuanVLChatModel" in hf_config.architectures:
        convert_checkpoint_from_transformers_to_megatron_yuan_vl(hf_model, model[0].module, hf_config)
        ...
        print(f'hf_config.architectures is {hf_config.architectures}')
...
if __name__ == "__main__":
    args = _init_args()
    convert_hf_to_mcore(
        args.hf_model_path, args.output_path, args.use_cpu_initialization, args.test, args.trust_remote_code, args.model_type
    )
