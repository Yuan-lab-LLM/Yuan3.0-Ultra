...
class TaskRunner:
    def run(self, config):
        ...
        hf_processor_path= copy_to_local(config.actor_rollout_ref.model.hf_processor_path)

        # instantiate tokenizer
        from verl.utils import hf_processor, hf_tokenizer

        tokenizer = hf_tokenizer(local_path)
        if hf_processor_path != None:
            processor = hf_processor(hf_processor_path, use_fast=True)  # used for multimodal LLM, could be none
        else:
