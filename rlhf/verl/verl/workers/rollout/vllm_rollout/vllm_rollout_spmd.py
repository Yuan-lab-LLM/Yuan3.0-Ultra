...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        assert pipeline_parallel_size == 1, f"now sync not support pipeline_parallel, pipeline_parallel={pipeline_parallel_size}"
        ...
        import os
        if os.getenv("VLLM_USE_V1", "0") != "1" and \
        ...
        self.eod_token_id = tokenizer.encode('<eod>')
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch:
            multi_modal_data_backup = non_tensor_batch["multi_modal_data"]
            vllm_inputs = []
            for raw_prompt_ids, multi_modal_data in zip(
                non_tensor_batch.pop("raw_prompt_ids"), non_tensor_batch.pop("multi_modal_data"), strict=True
            ):
                if multi_modal_data != None:
                    if 'image' in multi_modal_data and multi_modal_data['image'] == None:
                        vllm_inputs.append({"prompt_token_ids": raw_prompt_ids})
                    else:
                        vllm_inputs.append({"prompt_token_ids": raw_prompt_ids, "multi_modal_data": multi_modal_data})
                else:
                    vllm_inputs.append({"prompt_token_ids": raw_prompt_ids})
            non_tensor_batch["multi_modal_data"] = multi_modal_data_backup
        else:
            vllm_inputs = [
        ...
        with self.update_sampling_params(**kwargs):
            ...
            # 对 response 补 <pad> ，长度为 response_length
            ...
            # 拼接后 seq 的长度为 max_prompt_length + response_length
...
class vLLMAsyncRollout:
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.total_time_stats = 0.0
        self.total_response_length = 0
        self.total_rollout_times = 0
    ...
    def _print_message(self):
        if self.total_rollout_times == 0:
            return
        print(f"socket {self.socket} total cost: {self.total_time_stats} "
              f" rollout times: {self.total_rollout_times} "
              f" total response length: {self.total_response_length} ", flush=True)
        self.total_rollout_times = 0
        self.total_response_length = 0
        self.total_time_stats = 0.0
    ...
    async def _loop_forever(self):
        while True:
            ...
            start_tm = time.time()
            result = await self.execute_method(method, *args, **kwargs)
            if unique_reply_rank is None or unique_reply_rank == self.rpc_rank:
                ...
                if method == 'execute_model':
                    end_tm = time.time()
                    self.total_time_stats += end_tm - start_tm
                    self.total_rollout_times += 1
                    self.total_response_length += len(result.req_ids)
            else:
                await self.socket.send(pickle.dumps(None))
    ...
    async def execute_method(self, method: str | bytes, *args, **kwargs):
