...
class ExternalZeroMQDistributedExecutor(Executor):
    ...
    def _init_executor(self) -> None:
        ...
        self.send_complete_events = []
        self.send_thread_pools = {}
        self.rev_thread_pools = {}
        ...
        for index, address in enumerate(addresses):
            ...
            self.send_complete_events.append(send_complete)
            if self.max_concurrent_batches > 1:
                self.send_thread_pools[socket] = ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"send_{index}")
                self.rev_thread_pools[socket] = ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"rev_{index}")
    ...
    def collective_rpc(
        ...
        def send_submit(socket, recv_complete, send_complete, data ):
            try:
                recv_complete.wait()  # Wait for previous recv to complete
                recv_complete.clear() # Block future sends
                socket.send(data)
        ...
        for socket, recv_complete, send_complete in zip(self.sockets, self.recv_complete_events, self.send_complete_events):
            if non_block:
                self.send_thread_pools[socket].submit(send_submit, socket, recv_complete, send_complete, message)
            else:
                send_submit(socket, recv_complete, send_complete, message)

        def recv_response(socket: zmq.Socket, recv_complete, send_complete):
            try:
                send_complete.wait()   # Wait for previous send to complete
                send_complete.clear()  # Block future recv
        ...
        outputs = []
        for i, (socket, recv_complete, send_complete) in enumerate(zip(self.sockets, self.recv_complete_events, self.send_complete_events)):
            if non_block:
                val = self.rev_thread_pools[socket].submit(recv_response, socket, recv_complete, send_complete)
            else:
                val = recv_response(socket, recv_complete, send_complete)
            # non-reply rank
...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def init_engine(self):
        ...
        lm_head_dtype = config["lm_head_dtype"]
        assert lm_head_dtype in ["bfloat16", "float32", "float16"], f"lm_head_dtype must be bfloat16 or float32 or float16, now get {lm_head_dtype}"
    ...
    async def generate(self, prompt_ids: list[dict[str, Any]], sampling_params: dict[str, Any], request_id: str) -> list[int]:
        # max_tokens = self.max_model_len - len(prompt_ids)
        # sampling_params = SamplingParams(max_tokens=max_tokens, **sampling_params)
        sampling_params = SamplingParams(**sampling_params)
        # prompt = TokensPrompt(prompt_token_ids=prompt_ids)
        if "multi_modal_data" in prompt_ids[0]:
            prompt = TokensPrompt(prompt_token_ids=prompt_ids[0]["prompt_token_ids"], multi_modal_data=prompt_ids[0]["multi_modal_data"])
        else:
            prompt = TokensPrompt(prompt_token_ids=prompt_ids[0]["prompt_token_ids"])
