...
lock = threading.Lock()
LLM_PROCESS_METHOD_FLAG = ['llm_math','llm_choice']
...
def qwen_vllm_inference(input_text, api, model_path="gpt-oss-20b", max_tokens=3064, level="medium"):
    ...
    system_prefix = "You are an AI assistant specialized in analyzing text to locate and extract the precise segments that contain the complete and standard answer to a given question."
    ...
    think_text = response.choices[0].message.reasoning_content
...
def process_with_multithreading_map(split_think_text_merge, question, ans_end, api, answer_location_prompt, reward_method):
    ...
    # 使用map处理
    global_executor = futures.ThreadPoolExecutor(max_workers=min(20, len(split_think_text_merge)))
    try:
...
def process_line(question_ord, data):
    ...
    try:
        ...
        if data["reward_method"] in ["llm_choice","mllm_choice"] or re.findall(r'^[A-Z]$', answer_true_new.strip()):
...
def thread_func(questions):
    print(f"Hello World!!!!!")
    results = []
    with futures.ThreadPoolExecutor(max_workers=min(128, len(questions))) as executor:
...
def process_input_lst(questions):
    ...
    num_processes = 80 # num_process 需整除节点数*4
...
def verify_score(marked_txt, acc_score, v_exp, v_max):
    ...
    if (v == 0.0) and (acc_score < 0) and ("<1st_answer>" not in marked_txt):
        R_ver = -1.0
    else:
        if acc_score > 0 :
            if v <= v_exp:
