...
def math_verify_reward(content,sol):
    ...
    if gold_parsed == []:
        print(f"Warning: gold_parsed is None, sol:{[sol]}")
        return False
...
def check_force_code(ans_text, unittest_lst, check_num = 5):
    ...
    for test_case in check_unittest:
        ...
        try:
            ...
            du_cmd = ['timeout','15s', 'python3', f'./code_tmp/middle_py/{unique_id}.py']
...
def check_correctness(code_text, unittest_lst, check_num = 5):
    check_num = len(unittest_lst)
    if not code_text or check_num < 1:
        return float(0)
...
def exec_unittest(middle_text, only_unittest):
    ...
    if not only_code:
        return float(0)
    if type(only_unittest) is list:
        check_result = check_correctness(only_code, only_unittest)
    else:
        only_unittest = only_unittest.replace('<n>', '\n').strip().split('\n')
        only_unittest = [line for line in only_unittest if line.strip() != '']
        check_result = check_correctness(only_code, only_unittest)
    return check_result
...
def llm_score_code(ans_text, unittest_text):
    if "assert" in unittest_text: # humaneval/mbpp/leetcode
        ans_text = ans_text.replace('<eop>', '')
        try:
            unittest_text = ast.literal_eval(unittest_text) # 如果出现字符串列表，则进行解析，否则报错
        except:
            pass
...
def code_eval(reward_input):
    one_json={"response":reward_input["response"],"answer":reward_input["answer"]}
    try:
        url = f'http://{reward_input["reward_vllm_api"]}/code_eval'
        response = requests.post(url, json=one_json, timeout=120)
        one_json=json.loads(response.text)
        return one_json
    except requests.Timeout:
        print("requests-Timeout..")
        one_json['rl_judge_code_result'] = 'requests.Timeout'
    except requests.ConnectionError:
        print("requests-ConnectionError")
        one_json['rl_judge_code_result'] = 'requests.ConnectionError'
        return one_json
    except requests.RequestException as e:
        print("requests-Error")
        one_json['rl_judge_code_result'] = 'requests.RequestException'
        return one_json
    except Exception as e:
        print("发生错误:", str(e))
        one_json['rl_judge_code_result'] = str(e)
        return one_json
...
def format_reward(reward_output, lowest_score=-1.0):
    ...
    if _close_count > 0 or _open_count > 0 or _sep_count > 0:
        return lowest_score
...
def format_reward_short(reward_output, lowest_score=-1.0):
    response = reward_output['response']
