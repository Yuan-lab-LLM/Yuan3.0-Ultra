...
from verl.utils.reward_score.eval_rag.qa_em import compute_score_model, compute_score_summary
...
def llm_process(data,args,q_ord):
    ...
    elif reward_method in LLM_PROCESS_METHOD_FLAG:
        _input={**reward_input, **score}
        result=process_line(q_ord, _input)
        data[0]=result
    
    elif reward_method == "rag_summary":
        iscore=compute_score_summary(reward_input)
        score["acc_score"] = iscore
        score["reward_score"]=score["acc_score"]
