...
def compute_reward_score(
    ...
    overlong_buffer_enable = True if overlong_buffer_cfg and overlong_buffer_cfg.enable and (reward_method not in LLM_PROCESS_METHOD_FLAG) else False
    ...
    if reward_method in MLLM_METHOD_FLAG:
        ...
        # 只惩罚问答部分，而且惩罚1.75k长度区间
        elif ('mllm' in reward_method) and (not enable_thinking_flag):
            res = calculate_overlong_reward(res, overlong_buffer_len=2048, max_resp_len=6*1024, response_length=valid_response_length, overlong_buffer_enable=overlong_buffer_enable)
    ...
    # For LLM
        ...
        if reward_method == "llm_tool_reward":
            ...
            res = calculate_overlong_reward(res, overlong_buffer_len=1024, max_resp_len=5.5*1024, response_length=valid_response_length, overlong_buffer_enable=overlong_buffer_enable)
    # For General
    ...
    # For Rag
        ...
        reward_input["answer"]=try_parse_dict(reward_input["answer"])
        overlong_buffer_len = 1024 if reward_method in ["rag_ifeval", "rag_mmtab"] else 2048
        res = rag_reward_model(reward_input,args)
        res = calculate_overlong_reward(res, overlong_buffer_len=overlong_buffer_len, max_resp_len=4096, response_length=valid_response_length, overlong_buffer_enable=overlong_buffer_enable)
    # For SQL
