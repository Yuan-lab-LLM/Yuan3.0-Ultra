...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        assert len(data_files) == 1 and data_files[0].endswith(".txt"), f"检查输入数据：{data_files}"
        dataset_name = data_files[0]
        print(f"Get dataset {dataset_name}")
        data_files = load_paths(data_files[0])
        print(f"Get dataset number {len(data_files)}")
        print(data_files)
