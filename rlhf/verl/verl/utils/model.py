...
def compute_yuanvl_position_id_with_mask(mask, raw_prompt_ids):
    position_ids = torch.zeros_like(mask)
    prompt_position_ids= torch.range(start=0, end=len(raw_prompt_ids))
    position_ids[0][-len(raw_prompt_ids)-1:].copy_(prompt_position_ids)
    return position_ids
...
def _load_hf_model(config, model_config, is_value_model, local_cache_path):
    ...
    with init_context(), warnings.catch_warnings():
        ...
        if "mistral7b-rm" in config.model.path:
            ...
            is_value_model = True
        elif 'yuanvl' in config.model.path:
            ...
            state_dict = model.state_dict()
        elif 'yuan' in config.model.path:
...
def load_megatron_model_weights(
    ...
    if 'yuan' in config.model.path and config.model.load_mode == 'fast':
        from verl.models.mcore.loader import fast_yuanvl_loader
        print(f'============= fast loading ==================', flush=True)
        fast_yuanvl_loader(
            hf_model_path=config.model.path,
            wrapped_models=parallel_model[0],
...
def load_megatron_gptmodel_weights(
    ...
    if 'yuan' in config.model.path and config.model.load_mode == 'fast':
        ...
        fast_yuanvl_loader(
            hf_model_path=config.model.path,
            wrapped_model=unwrap_model(parallel_model)[0],
            config=config.llm_config if hasattr(config, 'llm_config') else config,
            params_dtype=params_dtype,
            is_value_model=is_value_model,
        )
        ...
        load_state_dict_to_megatron_gptmodel(
            state_dict=state_dict,
            wrapped_models=parallel_model,
            config=model_config.llm_config if hasattr(model_config, 'llm_config') else model.config,
            params_dtype=params_dtype,
            is_value_model=is_value_model,
        )
