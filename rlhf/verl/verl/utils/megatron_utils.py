...
from megatron.core.transformer import TransformerConfig as MTransformerConfig
...
@dataclass
class TransformerConfig(MTransformerConfig):
    lf_conv2d_group: int = 0
    lf_conv2d_num_pad: int = 0
    lf_conv2d_add_bias: bool = False
    moe_config: Dict = None
    per_layer_experts_blocks: List = None
    num_moe_experts: int = None
    num_key_value_heads: int = None
    use_attention_router: bool = False
    rotary_base: int = 0
    ...
    def __post_init__(self):
        ...
        #self.lf_conv2d_add_bias = True
        self.per_layer_experts_blocks = None
        if self.moe_config != None:
            if 'per_layer_experts_blocks' in self.moe_config.keys():
                self.per_layer_experts_blocks = self.moe_config['per_layer_experts_blocks']
        if self.num_key_value_heads == None:
            self.num_key_value_heads = self.num_attention_heads
        if self.padded_vocab_size == None:
...
def get_model(
    ...
    if wrap_with_ddp:
        ddp_models = []
        main_grads_dtype = optim_config.main_grads_dtype
        if main_grads_dtype == 'fp32':
            grad_reduce_in_fp32 = True
        elif main_grads_dtype == 'bf16':
            grad_reduce_in_fp32 = False
        else:
            raise ValueError(f"Unsupported main_grads_dtype: {main_grads_dtype}. "
...
def convert_config(hf_config: PretrainedConfig, megatron_config) -> TransformerConfig:
    ...
    qkv_bias = True if "Qwen2ForCausalLM" in (hf_config.architectures or []) else getattr(hf_config, "attention_bias", False)
    overlap_p2p_comm = (
...
def init_megatron_optim_config(optim_config: dict) -> OptimizerConfig:
    ...
    use_precision_aware_optimizer = optim_config.use_precision_aware_optimizer
    optimizer_cpu_offload = optim_config.optimizer_cpu_offload
    use_offload_precision_aware_optimizer = optim_config.use_offload_precision_aware_optimizer

    main_params_dtype = map_dtype(optim_config.main_params_dtype)
    assert main_params_dtype == torch.float32 or main_params_dtype == torch.float16, f"main_params_dtype should in ['fp32', 'fp16'], but get {optim_config.main_params_dtype}"

    exp_avg_dtype = map_dtype(optim_config.exp_avg_dtype)
    assert exp_avg_dtype == torch.float32 or exp_avg_dtype == torch.float16 or exp_avg_dtype == torch.uint8, f"exp_avg_dtype should in ['fp32', 'fp16', 'fp8'], but get {optim_config.exp_avg_dtype}"

    exp_avg_sq_dtype = map_dtype(optim_config.exp_avg_sq_dtype)
    assert exp_avg_sq_dtype == torch.float32 or exp_avg_sq_dtype == torch.float16 or exp_avg_sq_dtype == torch.uint8, f"exp_avg_sq_dtype should in ['fp32', 'fp16', 'fp8'], but get {optim_config.exp_avg_sq_dtype}"

    main_grads_dtype = map_dtype(optim_config.main_grads_dtype)
    assert main_grads_dtype == torch.float32 or main_grads_dtype == torch.bfloat16, f"main_grads_dtype should in ['fp32', 'bf16'], but get {optim_config.main_grads_dtype}"
    ...
    config = OptimizerConfig(
        ...
        use_offload_precision_aware_optimizer=use_offload_precision_aware_optimizer,
        optimizer_offload_fraction=optim_config.get("optimizer_offload_fraction", 1.0),
        adam_beta1=optim_config.get("adam_beta1", 0.9),
        adam_beta2=optim_config.get("adam_beta2", 0.95),
        adam_eps=optim_config.get("adam_eps", 1e-15),
    ...
    if torch.distributed.get_rank() == 0:
        print('optimizer_config_check:', config)
    return config
                ...
                for buffer in buffers:
                    ...
                    if buffer.param_data.storage().size() > 0:
                        if hasattr(buffer.param_data, 'cpu_data'):
                            buffer.param_data.cpu_data.resize_(buffer.param_data_size)
                            buffer.param_data.cpu_data.copy_(buffer.param_data.data, non_blocking=True)
                        else:
    ...
    for model_chunk in models:
        if isinstance(model_chunk, DDP):
            ...
            for buffers in model_chunk_all_buffers:
                for buffer in buffers:
                    if buffer.param_data.storage().size() == 0:
                        buffer.param_data.cpu_data.resize_(0)
...
def default_tp_concat_fn(
    ...
    elif 'linear_qk' in name:
        q_lst = []
        k_lst = []

        num_attention_heads = model_config.num_attention_heads
        num_key_value_heads = model_config.num_key_value_heads
        num_query_groups_per_partition = num_attention_heads // train_tp_size

        # 兼顾在Yuan模型中使用GQA
        assert num_attention_heads % num_key_value_heads == 0
        num_querys_per_kv = num_attention_heads // num_key_value_heads

        kv_size_per_tp = infer_params[0].shape[0] // (num_querys_per_kv + 1)
        qk_sizes = [kv_size_per_tp * num_querys_per_kv, kv_size_per_tp]
        for linear_qk in infer_params:
            q, k = linear_qk.split(qk_sizes)
            q_lst.append(q)
            k_lst.append(k)
        q = torch.cat(q_lst, dim=0)
        k = torch.cat(k_lst, dim=0)
        infer_params = torch.cat((q, k), dim=0)
        '''
        elif 'linear_proj' in name:
            # concat tensor
            infer_params = torch.cat(infer_params, dim=tp_utils.get_tensor_parallel_partition_dim(train_params))

        elif 'linear_v' in name:
            # concat tensor
            infer_params = torch.cat(infer_params, dim=tp_utils.get_tensor_parallel_partition_dim(train_params))
            ...
            infer_params = torch.cat(infer_params, dim=tp_utils.get_tensor_parallel_partition_dim(train_params))
        '''
    elif 'imagemlp.linear_fc1.weight' in name:
        up_lst = []
        gate_lst = []
        for linear_fc1 in infer_params:
            up, gate = linear_qk.chunk(2)
            up_lst.append(up)
            gate_lst.append(gate)
        up = torch.cat(up_lst, dim=0)
        gate = torch.cat(gate_lst, dim=0)
        infer_params = torch.cat((up, gate), dim=0)
...
def per_tensor_generator(
    ...
    etp_size = mpu.get_expert_tensor_parallel_world_size() # 每个专家的张量并行
    ...
    # we need first make all pipeline rank get self model information
    # then we can broadcast the model information to all other pipeline rank
    # layer_list_meta is a list of tuple (pp_rank, scan_vpp_idx, idx, name)
    meta_info = []
    for scan_vpp_idx in range(vpp_size):
        existing_keys = set()
        model = unwrap_model(actor_module[scan_vpp_idx])
    ...
    for cur_pp_rank, scan_vpp_idx, idx, name in layer_list_meta:
        ...
        if ".mlp.experts.linear_fc" in cur_name and ep_size > 1:
            ...
            if isinstance(weight_converter.mcore_config.per_layer_experts_blocks, list):
                pattern = r'decoder\.layers\.(\d+)'
                match = re.search(pattern, cur_name)
                layer_id = int(match.group(1))
                num_experts_per_rank = weight_converter.mcore_config.per_layer_experts_blocks[layer_id] // ep_size
            else:
            ...
            for name, param in zip(global_expert_names, infer_params, strict=True):
                if etp_size > 1:
                    ...
                    params = [param]
                # merge all experts along tp dim
        ...
        # tp all gather
        if tp_utils.is_tensor_parallel_param(broad_pp_tensor) and tp_size > 1:
    ...
    if config.pipeline_model_parallel_size > 1:
        if (
            ...
            if mpu.get_virtual_pipeline_model_parallel_world_size() is not None:
                ...
                if isinstance(config.per_layer_experts_blocks, list):
                    offset = sum(config.pipeline_blocks[:pipeline_rank])
                else:
