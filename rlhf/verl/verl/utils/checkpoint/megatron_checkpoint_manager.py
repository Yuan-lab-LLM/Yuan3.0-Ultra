...
class MegatronCheckpointManager(BaseCheckpointManager):
    ...
    def generate_model_state_dict(self):
    ...
    def generate_optimizer_state_dict(self):
        # For save dist checkpointing
        state_dict = {}
        ...
        optimizer_sharded_states = self.optimizer.sharded_state_dict(state_dict, is_loading=True)
        for vpp_rank, model in enumerate(self.model):
            if len(self.model) > 1:
                mpu.set_virtual_pipeline_model_parallel_rank(vpp_rank)
                key = f"model{vpp_rank}" if len(self.model) > 1 else "model"
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        if self.checkpoint_load_mode== 'total':
            ...
            assert self.use_dist_checkpointing, f'split save checkpoint only support use_dist_checkpointing'
            if self.should_load_model:
                sharded_state_dict = self.generate_model_state_dict()
                log_with_rank(f"Generated state dict for saving: {sharded_state_dict.keys()}", rank=self.rank, logger=logger)
                for vpp_rank, model in enumerate(self.model):
                    if len(self.model) > 1:
                        model_i_keys = sharded_state_dict[f"model{vpp_rank}"].keys()
                ...
                model_dist_checkpoint_path = os.path.join(dist_checkpoint_path, 'model')
                # Load Dist Checkpointing
                state_dict = load_dist_checkpointing(
                    sharded_state_dict=sharded_state_dict,
                    ckpt_dir=model_dist_checkpoint_path,
                )
                assert "model" in state_dict or any(
                    f"model{vpp_rank}" in state_dict for vpp_rank in range(len(self.model))
                ), f"Model state dict not found in {state_dict.keys()}. Please check the checkpoint file {local_path}."
                ...
                gc.collect()
            if self.should_load_optimizer:
                sharded_state_dict = self.generate_optimizer_state_dict()
                log_with_rank(f"Generated state dict for saving: {sharded_state_dict.keys()}", rank=self.rank, logger=logger)
                for vpp_rank, model in enumerate(self.model):
                    if len(self.model) > 1:
                        model_i_keys = sharded_state_dict[f"model{vpp_rank}"].keys()
                ...
                optimizer_dist_checkpoint_path = os.path.join(dist_checkpoint_path, 'optim')
                # Load Dist Checkpointing
                state_dict = load_dist_checkpointing(
                    sharded_state_dict=sharded_state_dict,
                    ckpt_dir=optimizer_dist_checkpoint_path,
                )
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.use_dist_checkpointing:
            if self.checkpoint_save_mode== 'total':
                ...
                if self.should_save_model:
                    state_dict = self.generate_model_state_dict()
                    local_mkdir_safe(os.path.join(dist_checkpoint_path, "model"))
                    # Start Async save if enabled
                    async_save_request = save_dist_checkpointing(
                        sharded_state_dict=state_dict,
                        ckpt_path=os.path.join(dist_checkpoint_path, 'model'),
                        async_save=self.checkpoint_config.async_save,
                    )
                    torch.cuda.synchronize()
                    gc.collect()
                ...
                if self.should_save_optimizer:
                    state_dict = self.generate_optimizer_state_dict()
                    local_mkdir_safe(os.path.join(dist_checkpoint_path, "optim"))
                    async_save_request = save_dist_checkpointing(
                        sharded_state_dict=state_dict,
                        ckpt_path=os.path.join(dist_checkpoint_path, 'optim'),
                        async_save=self.checkpoint_config.async_save,
                    )
                    torch.cuda.synchronize()
                    gc.collect()
