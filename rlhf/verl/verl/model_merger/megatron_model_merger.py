...
def distribute_layers(layer_num, pp_size, uniform_mode="absolute"):
    """
    uniform_mode: 
        "absolute" - 最均匀分配（允许中间阶段不同，差异≤1）
        "middle_fixed" - 保持中间阶段相同（与原代码逻辑一致，但修复bug）
    """
    if uniform_mode == "middle_fixed":
        # 保持中间pp_size-2个阶段完全相同（兼容原逻辑）
        middle_size = pp_size - 2
        max_middle = layer_num // middle_size
        best_strategy = None
        min_diff = float('inf')
        
        for m in range(1, max_middle + 1):
            remaining = layer_num - m * middle_size
            if remaining < 2:
                continue
            ...
            # 计算最大差异（衡量均匀度）
            diff = max(abs(first - m), abs(last - m), abs(first - last))
            
            if diff < min_diff:
                min_diff = diff
                best_strategy = [first] + [m] * middle_size + [last]
        
        return best_strategy
    
    else:  # "absolute" - 最均匀分配，推荐
        base = layer_num // pp_size
        remainder = layer_num % pp_size
        
        # remainder个阶段分base+1层，其余分base层
        res = [base + 1] * remainder + [base] * (pp_size - remainder)
        
        # 可选：把较多的层分散到中间，让首尾也保持较小
        # 例如把前remainder个阶段中的部分移到中间位置，避免开头堆积
        # 但均匀度指标（最大-最小）不变
...
def get_dynamic_pipeline_shards(layer_num: int, pp_size: int) -> list[int]:
    ...
    res = distribute_layers(layer_num, pp_size, "absolute")
    assert sum(res) == layer_num
    return res
...
class MegatronModelMerger(BaseModelMerger):
    ...
    def __init__(self, config: ModelMergerConfig):
        ...
        if "YuanVLChatModel" in self.hf_config.architectures:
            print('use YuanVLChatModel mapping...')
            self.params_mapping = {
                # embedding
                ...
                "imagemlp.linear_fc1.weight": "imagemlp.linear_fc1.weight",
                "imagemlp.linear_fc2.weight": "imagemlp.down_proj.weight",
                # input layer norm
                "input_layernorm.weight": "input_layernorm.weight",
                # attn
                "self_attention.linear_proj.weight": "self_attn.o_proj.weight",
                "self_attention.linear_v.weight": "self_attn.v_proj.weight",
                "self_attention.linear_qk.weight": "self_attn.get_query_key.weight",
                "self_attention.lf_gate.conv1.weight": "self_attn.lf_gate.conv1.weight",
                "self_attention.lf_gate.conv2.weight": "self_attn.lf_gate.conv2.weight",
                "self_attention.lf_gate.conv1.bias": "self_attn.lf_gate.conv1.bias",
                "self_attention.lf_gate.conv2.bias": "self_attn.lf_gate.conv2.bias",
                "self_attention.lf_gate.output_layernorm.weight": "self_attn.lf_gate.output_layernorm.weight",
                # mlp
                "pre_mlp_layernorm.weight": "post_attention_layernorm.weight",
                "mlp.router.attention_router.weight": "mlp.router.query_key_value.weight",
                "mlp.router.weight": "mlp.router.weight",
                "mlp.experts.local_experts": "mlp.experts",
                # output
    ...
    def _load_state_dicts(self, model_ckpt_path: str) -> dict[str, Any]:
        ...
        # init hf config
        self.pipeline_shards = get_dynamic_pipeline_shards(self.hf_config.llm_config.num_hidden_layers, self.world_size)
        print(f"Pipeline shards: {self.pipeline_shards}, total layers: {sum(self.pipeline_shards)}")

        if hasattr(self.hf_config, 'llm_config'):
            print(f'self.hf_config.llm_config.tie_word_embedding: {self.hf_config.llm_config.tie_word_embeddings}')
            tie_word_embeddings = getattr(self.hf_config.llm_config, "tie_word_embeddings", False)
            self.hf_config.llm_config.tie_word_embeddings = tie_word_embeddings
        else:
            tie_word_embeddings = getattr(self.hf_config, "tie_word_embeddings", False)
            self.hf_config.tie_word_embeddings = tie_word_embeddings
        if torch.distributed.get_rank() == 0:
            print(f'tie_word_embeddings: {tie_word_embeddings}')
        tf_config = hf_to_mcore_config(
            self.hf_config,
            torch.bfloat16,
            pipeline_model_parallel_method='block',
            pipeline_model_parallel_blocks=self.pipeline_shards
        )
    ...
    def _check_megatron_state_key(self, key: str) -> bool:
        ...
        skip_checking_keys = ["embedding.word_embeddings", "output_layer", "imagemlp"]
    ...
    def _split_tensors(
        ...
        """
        if "imagemlp.linear_fc1.weight" in key:
            # if the tensor is gate and proj
            gate_lst = []
            up_lst = []
            up, gate = tensor.chunk(2)
        ...
        for model_state_dict in model_state_dict_list:
            ...
            for key in keys:
                ...
                if "mlp.experts." in hf_name and ".weight" in hf_name:
                    name_prefix, name_last = hf_name.split("mlp.experts.")
                    expert_id, para_name, w_name = name_last.split(".")
                    if "linear_fc1" in para_name:
                        new_para_name = "w1"
                    elif "linear_fc2" in para_name:
                        new_para_name = "w2"

                    hf_name = f"{name_prefix}mlp.experts.{new_para_name}.{expert_id}.weight"
                ...
                if len(split_tensor) == 1:
                    ...
                    # split gate up
                    state_dict[hf_name.replace("linear_fc1", "gate_proj")] =  split_tensor[0]
                    state_dict[hf_name.replace("linear_fc1", "up_proj")] = split_tensor[1]
                shape_info = (
    ...
    def save_hf_model_and_tokenizer(self, merged_state_dict):
        if self.world_size == 1:
            raise RuntimeError(f"YuanVL not support world_size = 1 now!")

        from safetensors.torch import save_file

        layer_num = self.hf_config.llm_config.num_hidden_layers
        ...
        if self.rank == 0:
            target_vit_dir = Path(self.config.vit_dir)
            print('target_vit_dir = ', target_vit_dir)
            model_vit = AutoModel.from_pretrained(
                target_vit_dir,
        ...
        for i, keys in enumerate(keys_chunk):
            ...
            numel += sum(sd_to_save[k].numel() for k in sd_to_save)
            ...
            if self.rank == 0 and i == 0:
                for key, value in new_state_dict.items():
                    saves_indexes[key] = str(save_path.name)
            for k in keys:
    ...
    def merge_and_save(self):
        ...
        if self.config.load_mode == 'split':
            model_ckpt_path = os.path.join(model_ckpt_path, 'model')
        print(f'=============== self.config.load_mode: {self.config.load_mode} model_ckpt_path: {model_ckpt_path} ===================')
    ...
    def _replace_name(self, megatron_name: str, name_mapping: dict[str, str]) -> str:
        for m_name, v_name in name_mapping.items():
            ...
            megatron_name = megatron_name.replace("decoder", "language_model.model")
