...
def _get_base_transformer_config(
    ...
    base_config = {
        ...
        "attention_dropout": hf_config.attention_dropout,  # flash_attn_drop
        ...
        "moe_token_dispatcher_type": "alltoall" if not hasattr(hf_config, "moe_token_dispatcher_type") else hf_config.moe_token_dispatcher_type,
        "per_layer_experts_blocks": None
    }
...
def hf_to_mcore_config_dense(
    ...
    qk_layernorm = True if "Qwen3ForCausalLM" in hf_config.architectures else getattr(hf_config, "qk_layernorm", False)
...
def hf_to_mcore_config_yuan(
    hf_config: PretrainedConfig, dtype: torch.dtype, **override_transformer_config_kwargs
) -> TransformerConfig:
    # YuanForConditionalGeneration
    use_attention_router = False
    if hasattr(hf_config, 'moe_config'):
        if hf_config.moe_config['router_type'] == 'attn_router':
            use_attention_router = True
        if 'per_layer_experts_blocks' in hf_config.moe_config:
            per_layer_experts_blocks = hf_config.moe_config['per_layer_experts_blocks']
            moe_num_experts = 1 
        elif 'moe_num_experts' in hf_config.moe_config:
            moe_num_experts = hf_config.moe_config['moe_num_experts']
            per_layer_experts_blocks = None

    padded_vocab_size = math.ceil(hf_config.vocab_size / 128) * 128
    args = _get_base_transformer_config(
        hf_config=hf_config,
        dtype=dtype,
        num_layers=hf_config.num_hidden_layers,
        lf_conv2d_group=hf_config.lf_conv2d_group,
        lf_conv2d_num_pad=hf_config.lf_conv2d_num_pad,
        moe_config=hf_config.moe_config,
        rotary_base=hf_config.rotary_base,
        num_moe_experts=moe_num_experts,
        add_bias_linear=False,
        per_layer_experts_blocks=per_layer_experts_blocks,
        use_attention_router=use_attention_router,
        num_attention_heads=hf_config.num_attention_heads,
        ...
        rotary_percent=hf_config.rotary_percent,
        use_rope_scaling=hf_config.use_rope_scaling,
    )
    # override_transformer_config_kwargs as kwargs shall never be none
    args.update(override_transformer_config_kwargs)
    args = check_and_disable_incompatible_configs(args)
...
def hf_to_mcore_config_yuan_vl(
    hf_config: PretrainedConfig, dtype: torch.dtype, **override_transformer_config_kwargs
) -> TransformerConfig:
    # YuanVLForConditionalGeneration
    assert hasattr(hf_config, "llm_config") and hasattr(hf_config, "vision_config"), f'yuan_vl hf_config must has vision_config and llm_config'
    assert hasattr(hf_config, 'clip_model_name') and hasattr(hf_config, 'clip_download_path'), f'yuan_vl hf_config must has clip_model_name and llm_config'
    use_attention_router = False
    if hasattr(hf_config.llm_config, 'moe_config'):
        if hf_config.llm_config.moe_config['router_type'] == 'attn_router':
            use_attention_router = True
        if 'per_layer_experts_blocks' in hf_config.llm_config.moe_config:
            per_layer_experts_blocks = hf_config.llm_config.moe_config['per_layer_experts_blocks']
            moe_num_experts = 1
        elif 'moe_num_experts' in hf_config.llm_config.moe_config:
            moe_num_experts = hf_config.llm_config.moe_config['moe_num_experts']
            per_layer_experts_blocks = None
        else:
            raise ValueError(f"{hf_config.llm_config.moe_config['moe_num_experts']}, {'per_layer_experts_blocks' in hf_config.llm_config.moe_config}, {'moe_num_experts' in hf_config.llm_config.moe_config}")
    else:
        moe_num_experts = 1
        per_layer_experts_blocks = None

    if hasattr(hf_config.llm_config, 'attention_projection_size'):
        if hasattr(hf_config.llm_config, 'num_key_value_heads'):
            hf_config.llm_config.head_dim = hf_config.llm_config.attention_projection_size // hf_config.llm_config.num_key_value_heads
        else:
            hf_config.llm_config.head_dim = hf_config.llm_config.attention_projection_size // hf_config.llm_config.num_attention_heads
            hf_config.llm_config.num_key_value_heads = hf_config.llm_config.num_attention_heads
    elif hasattr(hf_config.llm_config, 'num_key_value_heads'):
        hf_config.llm_config.head_dim = hf_config.llm_config.hidden_size // hf_confg.llm_config.num_attention_heads
        hf_config.llm_config.attention_projection_size = int(hf_config.llm_config.head_dim * hf_config.llm_config.num_key_value_heads)
    else:
        hf_config.llm_config.head_dim = hf_config.llm_config.hidden_size // hf_confg.llm_config.num_attention_heads
        hf_config.llm_config.attention_projection_size = hf_config.llm_config.hidden_size
        hf_config.llm_config.num_key_value_heads = hf_config.llm_config.num_attention_heads

    padded_vocab_size = math.ceil(hf_config.llm_config.vocab_size / 128) * 128
    args = _get_base_transformer_config(
        hf_config=hf_config.llm_config,
        dtype=dtype,
        imagemlp_recompute=hf_config.imagemlp_recompute,
        downsample_ratio=hf_config.downsample_ratio,
        clip_hidden_size=hf_config.vision_config.hidden_size,
        clip_visual_size=hf_config.vision_config.hidden_size,
        clip_model_name= hf_config.clip_model_name if hasattr(hf_config,"clip_model_name") else None,
        clip_download_path=hf_config.clip_download_path if hasattr(hf_config,"clip_download_path") else None,
        num_layers=hf_config.llm_config.num_hidden_layers,
        lf_conv2d_group=hf_config.llm_config.lf_conv2d_group,
        lf_conv2d_num_pad=hf_config.llm_config.lf_conv2d_num_pad,
        lf_conv2d_add_bias=hf_config.llm_config.lf_conv2d_add_bias,
        moe_config=hf_config.llm_config.moe_config,
        rotary_base=hf_config.llm_config.rotary_base,
        num_moe_experts=moe_num_experts,
        add_bias_linear=False,
        per_layer_experts_blocks=per_layer_experts_blocks,
        use_attention_router=use_attention_router,
        ...
        kv_channels=hf_config.llm_config.head_dim,
        padded_vocab_size=padded_vocab_size,
        vocab_size=hf_config.llm_config.vocab_size,
        max_position_embeddings=hf_config.llm_config.max_position_embeddings,
        rotary_percent=hf_config.llm_config.rotary_percent,
        use_rope_scaling=hf_config.llm_config.use_rope_scaling,
        use_lf_gate=hf_config.llm_config.use_lf_gate,
        num_query_groups=hf_config.llm_config.num_query_groups,
        use_yuanvl=True

    )
    # override_transformer_config_kwargs as kwargs shall never be none
    if 'attention_backend' in override_transformer_config_kwargs:
        attention_backend = override_transformer_config_kwargs['attention_backend']
        assert attention_backend == 'auto' or  attention_backend == 'flash', f"check attention_backend, attention_backend should equal auto or flash, but get {attention_backend}"
        override_transformer_config_kwargs['attention_backend'] = AttnBackend[attention_backend]

    args.update(override_transformer_config_kwargs)
    args = check_and_disable_incompatible_configs(args)

    print(f"Overridden TF init config: {args}")
    print(f"override_transformer_config_kwargs:{override_transformer_config_kwargs}")
