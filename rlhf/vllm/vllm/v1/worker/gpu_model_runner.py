...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        for value in self.requests.values():
            image_tokens_len = 0
            for x in value.mm_positions:
                image_tokens_len += x.length
        ...
        if sum(image_tokens_lens) > 0 and "Yuan" in self.model.__str__():
            ...
            for index, req_id in enumerate(self.input_batch.req_ids):
                ...
                image_tokens_len = sum([x.length for x in self.requests[req_id].mm_positions])
    ...
    def _dummy_run(
        ...
        with self.maybe_dummy_run_with_lora(self.lora_config,
            ...
            with self.maybe_randomize_inputs(input_ids), set_forward_context(
                    ...
                    num_tokens_across_dp=num_tokens_across_dp):
                if getattr(model, "language_model", None) and \
                        getattr(model.language_model, "get_lf_index", None):
                    model.language_model.get_lf_index(positions)
                outputs = model(
                    input_ids=input_ids,
                    positions=positions,
                    intermediate_tensors=intermediate_tensors,
