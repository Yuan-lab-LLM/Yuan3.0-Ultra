...
def get_tokenizer(
    ...
    **kwargs,
    ...
    if 'tokenizer_class' in kwargs:
        tokenizer_class = kwargs['tokenizer_class']
    else:
        tokenizer_class = None

    
    tokenizer: AnyTokenizer
    if tokenizer_class in ['YuanVLTokenizer', 'YuanTokenizer'] or \
            any(substring in tokenizer_name for substring in ['yuan', 'Yuan']):
        tokenizer = AutoTokenizer.from_pretrained(
                tokenizer_name, add_eos_token=False, add_bos_token=False,
                eos_token='<eod>', trust_remote_code=trust_remote_code)
        print('-----------------using yuan tokenizer-----------------')
    elif tokenizer_mode == "mistral":
