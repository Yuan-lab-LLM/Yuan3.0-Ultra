...
__launch_bounds__(TPB) __global__
    void fusedSoftmax(const float* input, const bool* finished, float* output,
                      const int num_cols) {
...
template <int TPB>
__launch_bounds__(TPB) __global__
    void fusedTopK(const float* input, const bool* finished, float* output,
...
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE) __global__
    void FusedTopkSoftmaxKernel(const float* input, const bool* finished,
  ...
  static constexpr int ELTS_PER_WARP = WARP_SIZE * VPT;        // 32 *4
  static constexpr int ROWS_PER_WARP = ELTS_PER_WARP / ELTS_PER_ROW;  // 16
  static constexpr int ROWS_PER_CTA =
      WARPS_PER_CTA * ROWS_PER_WARP;  // 16 * 4 = 64
  ...
  // printf("Block %d, Thread %d: Processing row %d, Reading data starting at
  // index %d\n", blockIdx.x, threadIdx.x, thread_row,
  ...
  // step2: softmax
  const float maxElem = output[thread_row * k];
  float threadData = 0;
  for (int k_idx = 0; k_idx < k; ++k_idx) {
    float expVal =
        exp(static_cast<float>(output[thread_row * k + k_idx]) - maxElem);
    threadData += expVal;
  }

  for (int k_idx = 0; k_idx < k; ++k_idx) {
    output[k * thread_row + k_idx] =
        exp(static_cast<float>(output[k * thread_row + k_idx]) - maxElem) *
        (1.f / threadData);
  }
}
...
void fusedTopkSoftmaxKernelLauncherHelper(const float* input,
                                          const bool* finished,
  ...
  FusedTopkSoftmaxKernel<VPT, EXPERTS, WARPS_PER_TB, BYTES_PER_LDG>
      <<<num_blocks, block_dim, 0, stream>>>(input, finished, output, num_rows,
                                             indices, k, start_expert,
                                             end_expert);
}

#define LAUNCH_FUSEDTOPKSOFTMAX(NUM_EXPERTS, WARPS_PER_TB)                \
  fusedTopkSoftmaxKernelLauncherHelper<NUM_EXPERTS, WARPS_PER_TB>(        \
      gating_output, nullptr, topk_weights, topk_indices,                 \
      num_tokens, topk, 0, num_experts, stream);

void fusedTopkSoftmaxKernelLauncher(
  ...
  switch (num_experts) {
    case 1:
      LAUNCH_FUSEDTOPKSOFTMAX(1, WARPS_PER_TB);
      break;
    case 2:
      LAUNCH_FUSEDTOPKSOFTMAX(2, WARPS_PER_TB);
      break;
    case 4:
      LAUNCH_FUSEDTOPKSOFTMAX(4, WARPS_PER_TB);
      break;
    case 8:
      LAUNCH_FUSEDTOPKSOFTMAX(8, WARPS_PER_TB);
      break;
    case 16:
      LAUNCH_FUSEDTOPKSOFTMAX(16, WARPS_PER_TB);
      break;
    case 32:
      LAUNCH_FUSEDTOPKSOFTMAX(32, WARPS_PER_TB);
      break;
    case 64:
      LAUNCH_FUSEDTOPKSOFTMAX(64, WARPS_PER_TB);
      break;
    case 128:
      LAUNCH_FUSEDTOPKSOFTMAX(128, WARPS_PER_TB);
      break;
    case 256:
      LAUNCH_FUSEDTOPKSOFTMAX(256, WARPS_PER_TB);
      break;
    default:
      TORCH_CHECK(softmax_workspace != nullptr, "softmax_workspace must be \
                  provided for num_experts that are not a power of 2.");
      static constexpr int TPB = 256;
      fusedTopK<TPB><<<num_tokens, TPB, 0, stream>>>(
          gating_output, nullptr, softmax_workspace, topk_indices, num_experts,
          topk, 0, num_experts);
      fusedSoftmax<TPB><<<num_tokens, TPB, 0, stream>>>(softmax_workspace,
...
void topk_softmax_v2(
    torch::Tensor& topk_weights,              // [num_tokens, topk]
    torch::Tensor& topk_indices,              // [num_tokens, topk]
    torch::Tensor& gating_output)             // input [num_tokens, num_experts]
{
  ...
  vllm::moe::fusedTopkSoftmaxKernelLauncher(
