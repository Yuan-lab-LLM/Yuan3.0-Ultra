...
def get_prompt(dataset):
    ...
    if 'textvqa' in dataset:
        ...
        prompt = 'Please directly answer the following question in short, '
...
if __name__ == '__main__':
    ...
    #['docvqa_val', 'ai2diagram_test', 'chartqa_test_augmented', 'chartqa_test_human','vqav2_val_5w']
    #parser.add_argument("--dataset", type=str, default='docvqa_val', help="")
    ...
    for dataset in args.dataset:
        ...
        for i in range(round_num):
            ...
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            result_file = case_name + '-' + str(batch_num) + '-' + dataset + '-short-oneword-' + time_prefix + '.json'
            result_file_multi = case_name + '-' + str(batch_num) + '-' + dataset + '-' + time_prefix + '_short_oneword_chat.json'
