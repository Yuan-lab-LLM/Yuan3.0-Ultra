...
def parse_args():
    ...
    parser.add_argument("--batch_size", type=int, default=96, help="Batch size for processing")
...
def batch_infer(prompts, client, temperature, top_p, output_path,lock):
    try:
        deal_prompts = 'Below are sql tables schemas paired with instruction that describes a task. Using valid SQL, write a response that appropriately completes the request for the provided tables.' + '\n' + '### Instruction:' + '\n' + prompts['question'] + '\n' + '### Input:' + '\n' + prompts['table'] + '\n' + '### Response:'
        ...
        # prompts['yuan_input'] = deal_prompts
        prompts['yuan_solution'] = response.choices[0].message.content
